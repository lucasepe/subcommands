...
type Commander struct {
	...
	topFlags  *pflag.FlagSet // top-level flags
...
// will be set as well.
func NewCommander(topLevelFlags *pflag.FlagSet, name string) *Commander {
	cdr := &Commander{
...
func (cdr *Commander) VisitAllImportant(fn func(*pflag.Flag)) {
...
func (cdr *Commander) VisitAll(fn func(*pflag.Flag)) {
	if cdr.topFlags != nil {
...
func explain(w io.Writer, cmd Command) {
	...
	subflags := pflag.NewFlagSet(cmd.Name(), pflag.PanicOnError)
...
func (h *helper) SetFlags(*pflag.FlagSet) {}
...
func (h *helper) Execute(_ context.Context, f *pflag.FlagSet, args ...interface{}) ExitStatus {
	switch f.NArg() {
	case 0:
...
func (flg *flagger) SetFlags(*pflag.FlagSet) {}
...
func (flg *flagger) Execute(_ context.Context, f *pflag.FlagSet, _ ...interface{}) ExitStatus {
	if f.NArg() > 1 {
		f.Usage()
		return ExitUsageError
	}
...
func (l *lister) SetFlags(*pflag.FlagSet) {}
func (l *lister) Usage() string {
	return `commands:
	Print a list of all commands.
`
}
func (l *lister) Execute(_ context.Context, f *pflag.FlagSet, _ ...interface{}) ExitStatus {
	if f.NArg() != 0 {
		f.Usage()
		return ExitUsageError
	}
...
// DefaultCommander is the default commander using pflag.CommandLine for flags
// and os.Args[0] for the command name.
var DefaultCommander *Commander

func init() {
	DefaultCommander = NewCommander(pflag.CommandLine, path.Base(os.Args[0]))
}
...
// initialized by pflag.Parse. It finds the correct subcommand and
